...
import { updateSuit } from '../services/api';
...
export const EditSuitModal: React.FC<EditSuitModalProps> = ({
  suit,
  isOpen,
  onClose,
  onSuccess,
  ...
  useEffect(() => {
    if (suit && isOpen) {
      setName(suit.name || '');
      setCode(suit.code || '');
      setCategory(suit.category || 'Slim Fit Italiano');
      setFabric(suit.fabric || '');
      setColor(suit.color || '');
      setSize(suit.size || '');
      setPrice(suit.price?.toString() || '');
      setCostPrice(suit.costPrice?.toString() || '');
      setStock(suit.stock?.toString() || '');
      setMinStock(suit.minStock?.toString() || '');
      setImage(suit.image || '');
    }
  }, [suit, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const numMinStock = parseInt(minStock, 10) || 0;
      const status = numStock <= numMinStock ? 'baixo_estoque' : 'disponivel';

      const updatedSuitData: Partial<Suit> = {
        name,
        code,
        category,
        fabric,
        color,
        colorHex: suit.colorHex || '#0f172a',
        size,
        price: parseFloat(price) || 0,
        costPrice: parseFloat(costPrice) || 0,
        stock: numStock,
        minStock: numMinStock,
        image: image || suit.image,
        status,
      };

      // 1. Send PUT to Express SQLite Backend (2ª AC)
      const ok = await updateSuit(suit.id, updatedSuitData);

      if (!ok) {
        throw new Error('Falha ao comunicar com o servidor');
      }

      // 2. Update Dexie IndexedDB cache as well
      await db.suits.update(suit.id, updatedSuitData);

      onToast('success', 'Terno atualizado com sucesso', `As alterações de "${name}" foram salvas com sucesso.`);
      onSuccess();
      onClose();
    } catch (error) {
      console.error('Erro ao atualizar terno:', error);
      onToast('error', 'Erro ao atualizar terno', (error as Error).message || 'Não foi possível salvar as alterações.');
    } finally {
